...
class StirlingSolver(BasePointSolver):
    ...
    def validate(self) -> InterpolationValidation:
        if self.n % 2 == 0:
            return InterpolationValidation(success=False, message="Number of points must be odd")

        if self.n < 5:
            return InterpolationValidation(success=False, message="Number of points must be at least 5")
    ...
    def solve(self) -> PointInterpolationResult:
        ...
        for i in range(1, n + 1):
            ...
            cur *= p
            cur *= (self._get_diff(2 * i - 1, -(i - 1)) + self._get_diff(2 * i - 1, -i)) / 2
            result_poly += cur
        ...
        return PointInterpolationResult(expr=sp.simplify(result_poly).expand(), y_value=float(str(y_value)))
    ...
    def _get_diff(self, order: int, index: int) -> float:
        """
    ...
    def _build_diff_table(self) -> list[list[float]]:
        """
